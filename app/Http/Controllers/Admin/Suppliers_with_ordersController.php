...
{check_permeission_Redirection(87);
   ...
   if (!empty($data)) {
     foreach ($data as $info) {
        ...
        $info->supplier_name = Supplier::where('suuplier_code', $info->suuplier_code)->value('name');
         $info->store_name = Stores::where('id', $info->store_id)->value('name');
   if ($info->updated_by > 0 and $info->updated_by != null) {
        $info->updated_by_admin = Admin::where('id', $info->updated_by)->value('name');
}}
}
   $suupliers = get_cols_where(new Supplier(), array('suuplier_code', 'name'), array('com_code' => $com_code), 'id', 'DESC');
    $stores = get_cols_where(new Stores(), array('id', 'name'), array('com_code' => $com_code, 'active' => 1), 'id', 'DESC');
...
{
  $com_code = auth()->user()->com_code;
  $admin_panel_setting=get_cols_where_row(new Admin_panel_settings (),array('is_set_batches_setting'),array("com_code"=>$com_code));
  if($admin_panel_setting['is_set_batches_setting']==0){
   return redirect()->route('admin.suppliers_orders.index')->with(['error' => 'عفوا يجب تحديد نوع الية عمل  الباتشات بالضبط العام ']);
...
{   try {
    ...
    $data['store_name'] = Stores::where('id', $data['store_id'])->value('name');
...
$admin_panel_setting=get_cols_where_row(new Admin_panel_settings (),array('batches_setting_type'),array("com_code"=>$com_code));
...
if($admin_panel_setting['batches_setting_type']==1){
...
$invoice_data['store_name'] = Stores::where('id', $invoice_data['store_id'])->value('name');
