...
use App\Http\Requests\CustomerEidReq;
use App\Models\Admin_panel_settings;
class CustomerController extends Controller

{
   public function index()
    {    check_permeission_Redirection(31);
 ...
 public function create()
    ...
    $data_insert['Name_enter_system']=$request->Name_enter_system;
    ...
    $customer_parent_account_number = get_field_value(new Admin_panel_settings(), "customer_parent_account_number", array('com_code' => $com_code));
...
public function update($id, CustomerEidReq $request)
{
try {
   $com_code = auth()->user()->com_code;
    $data = get_cols_where_row(new Customer(), array("id", "account_number", "customer_code"), array("id" => $id, "com_code" => $com_code));
   if (empty($data)) {
return redirect()->route('admin.Customer.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
}
...
$data_to_update['Name_enter_system']=$request->Name_enter_system;
