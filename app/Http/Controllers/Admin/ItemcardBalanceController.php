...
check_permeission_Redirection(111);
 ...
 if (!empty($allitemscardData)) {
  ...
  if(!empty($info->allBathces)){
    foreach($info->allBathces as $Det){
     $Det->store_name=get_field_value(new Stores(),"name",array("com_code"=>$com_code,"id"=>$Det->store_id));  
    if($info->does_has_retailunit==1){
...
$storesSearch=get_cols_where(new Stores(),array("id","name"),array("com_code"=>$com_code),'id','ASC');
...
$info->allQuantitySearch=Inv_itemcard_batches::where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where($field3,$operator3,$value3)->where($field4,$operator4,$value4)->where($field5,$operator5,$value5)->sum('quantity');     
$info->allBathces=Inv_itemcard_batches::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where($field3,$operator3,$value3)->where($field4,$operator4,$value4)->where($field5,$operator5,$value5)->orderBy('store_id','ASC')->orderBy('quantity','DESC')->get();
if(!empty($info->allBathces)){ 
foreach($info->allBathces as $Det){
$Det->store_name=get_field_value(new Stores(),"name",array("com_code"=>$com_code,"id"=>$Det->store_id));  
if($info->does_has_retailunit==1){
